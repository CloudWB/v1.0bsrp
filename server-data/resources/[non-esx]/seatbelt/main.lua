...
Fwv = function (entity)
    ...
    if car ~= 0 and (wasInCar or IsCar(car)) then
      ...
      if IsControlJustReleased(0, 305) and GetLastInputMethod(0) then
        beltOn = not beltOn 
        if beltOn then 
		  TriggerEvent("pNotify:SendNotification", {text = "Vous avez attaché votre ceinture", type = "success", timeout = 1400, layout = "centerLeft"})
		  ...
		  TriggerEvent("pNotify:SendNotification", {text = "Vous avez détacher votre ceinture", type = "error", timeout = 1400, layout = "centerLeft"})
