...
AddEventHandler('loffe_robbery:msgPolice', function(store, robber)
    ...
    ESX.ShowAdvancedNotification(Config.Shops[store].name, Translation[Config.Locale]['robbery'], Translation[Config.Locale]['cop_yes_no'], 'CHAR_CALL911', 4)
...
AddEventHandler('loffe_robbery:msgPolice', function(store, robber)
    local mugshot, mugshotStr = ESX.Game.GetPedMugshot(GetPlayerPed(GetPlayerFromServerId(robber)))
	ESX.ShowAdvancedNotification(Config.Shops[store].name, Translation[Config.Locale]['robbery'], Translation[Config.Locale]['cop_msg'], 'CHAR_CALL911', 4)
    UnregisterPedheadshot(mugshot)
    while true do
        local name = GetCurrentResourceName() .. math.random(999)
        AddTextEntry(name, '' .. Translation[Config.Locale]['set_waypoint'] .. '' .. Translation[Config.Locale]['hide_box'])
        DisplayHelpTextThisFrame(name, false)
        Wait(0)
    end
end)
